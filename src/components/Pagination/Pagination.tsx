...
//     ? Object.values(query)
//         .filter((item, index) => Object.keys(query)[index] != 'page')
//         .map((item, index) => {
//           return Object.keys(query)[index] + '=' + item;
//         })
//     : '';

const Pagination = ({ queryPath, path, activePage, onPageChange, totalPages }: PaginationProps) => {
  let pagesArray = [];
        ...
        <div className={clsx(styles.pagerWrapper)} data-testid={'pagination'}>
          {activePage !== undefined && activePage != 1 && (
            <>
              ...
              <button
                onClick={() =>
                  onPageChange(
                    path + '?' + queryPath + (queryPath == '' ? '' : '&') + 'page=' + (Number.parseInt(activePage) - 1),
                  )
                }
              ...
              {activePage === undefined ? (
                <>
                  {item === 1 ? (
                    ...
                    <button
                      onClick={() =>
                        onPageChange(path + '?' + queryPath + (queryPath == '' ? '' : '&') + 'page=' + item)
                      }
                ...
                <>
                  {activePage == item ? (
                    ...
                    >
                      <a className={styles.page}>{item}</a>
                    </button>
                  )}
                </>
              )}
          ...
          {(activePage === undefined || activePage != totalPages) && (
            <>
                ...
                onClick={() =>
                  onPageChange(
                    path +
                      ...
                      (queryPath == '' ? '' : '&') +
                      'page=' +
                      (activePage === undefined ? 1 : activePage + 1),
                  )
                }
              >
              ...
              <button
                onClick={() =>
                  onPageChange(path + '?' + queryPath + (queryPath == '' ? '' : '&') + 'page=' + totalPages)
                }
              >
